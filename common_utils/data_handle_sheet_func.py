			...
			for sheets_property in sheets_property_list:
				...
				if sheet_visibility == 0:  # 只读取可见的Sheet
					...
					if not df_worksheet.empty:
						...
						print(f'Getting from: "{table_stem}" {df_worksheet.shape[0]} rows')
	...
	def match_columns(self, complete_header_df, match_table_df):
		...
		for row in table_values:
			...
			if source_columns !='' and len(join_columns) > 0 and len(target_columns) > 0  :
				...
				complete_header_df = process_match_complete_table(complete_header_df, source_columns,target_columns)
			...
			if input_column != '' and output_column != '':
				...
				if input_column != '' and output_column != '':
					...
					if sort_value_str == 'desc':
						...
						sort_column_order_list.append(False)
					elif sort_value_str != '':
						sort_column_list.append(output_column)
		...
		if not time_process_df.empty and not data_df.empty:
			...
			for row in time_process_df.values:
				...
				if period in ['day','days','天','日','d']:
					data_df[original_period] = (enddate - data_df[time_column])/np.timedelta64(1,'D')
				elif period in [ 'week','weeks', '周','星期','w']:
					data_df[original_period] = (enddate - data_df[time_column])/np.timedelta64(1,'W')
				elif period in [ 'month','months', '月','月数','m']:
					data_df[original_period] = (enddate - data_df[time_column])/np.timedelta64(1,'M')
				elif period in [ 'year','years', '年','年数','y']:
					data_df[original_period] = (enddate - data_df[time_column])/np.timedelta64(1,'Y')
				elif period in [ 'hour','hours', '时','小时','h']:
					data_df[original_period] = (enddate - data_df[time_column])/np.timedelta64(1,'h')
